...
public interface EmployeeRepository extends JpaRepository<Employee, Integer>
{
	...
	@Query("update Employee e set e.status=?1 where e.id=?2  ") // set is a keyword to update one particular column
	@Modifying // data manipulation language (DML) 
	@Transactional // to enable auto commit
	public int updateempstatus(String status, int eid);
	// execute update - integer
	// delete - integer
	// count - long
